...
try:
    ...
    for p in data:
        ...
        if partido.lower() in vistos:
            continue
        ok, det = analiza_btts(home, away)
        if not ok:
            continue
...
mlb_pick = get_pick_espn("baseball/mlb", 8.5)
nfl_pick = get_pick_espn("football/nfl", 45.5)
nhl_pick = get_pick_espn("hockey/nhl", 6.5)

mlb_f = get_fallback("baseball/mlb")
nfl_f = get_fallback("football/nfl")
nhl_f = get_fallback("hockey/nhl")

mlb_txt = mlb_pick["partido"] if mlb_pick else (mlb_f[0] if mlb_f else futbol[2])
mlb_det = f"Avg {mlb_pick['avg']} Over {mlb_pick['pct']:.0f}%" if mlb_pick else "MLB"
nfl_txt = nfl_pick["partido"] if nfl_pick else (nfl_f[0] if nfl_f else futbol[3])
nfl_det = f"Avg {nfl_pick['avg']} Over {nfl_pick['pct']:.0f}%" if nfl_pick else "NFL"
nhl_txt = nhl_pick["partido"] if nhl_pick else (nhl_f[0] if nhl_f else futbol[3])
...
msg += f"1) MINI BTTS @{round(c1*c2,2)}\n- {futbol[0]} - BTTS SI @{c1} [{detalles[0]}]\n- {futbol[1]} - BTTS SI @{c2} [{detalles[1]}]\n\n"
msg += f"2) COMBI MIXTA @{round(c_mlb*c_nfl,2)}\n- {mlb_txt} - Over 8.5 @{c_mlb} [{mlb_det}]\n- {nfl_txt} - Over 45.5 @{c_nfl} [{nfl_det}]\n\n"
msg += f"3) FIJA 1.80\n- {futbol[2]} - Gana @{c_fija}\n\n"
msg += f"4) VALUE 2.0+\n- {nhl_txt} - Over 6.5 @{c_nhl} [{nhl_det}]\n\n"
msg += f"💵💰❤"
